...
- **Core payment policy (`PaymentPolicy` + `PaymentStore`).** Direct
  `PaymentClient` users can enforce a per-payment maximum, exact prospective
  rolling caps, and host/network/asset/recipient allowlists. SQLite reservations
  are atomic across processes and fail closed by default; signed payments with
  ambiguous outcomes remain reserved ([policy.py](arc_agent_pay/policy.py),
  [payment_store.py](arc_agent_pay/payment_store.py)).
